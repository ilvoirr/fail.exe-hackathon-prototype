...
export default function BearishDashboard() {
 ...
 const fetchData = async () => {
  ...
  return (
    <main className="h-screen w-screen bg-[#050505] text-white overflow-hidden relative font-sans selection:bg-red-500/30 flex flex-col p-6">
      
      {/* Background Ambience */}
      <div className="fixed top-[-20%] right-[-10%] w-[800px] h-[800px] bg-red-900/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="fixed bottom-[-20%] left-[-10%] w-[600px] h-[600px] bg-red-950/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      <header className="flex-none mb-6 flex items-end justify-between z-20">
        <div className="flex items-center gap-6">
          ...
          <div>
            <motion.h1 
              ...
              className="text-5xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-red-600 via-white to-red-500 leading-none"
            >
              BEAR<span className="text-red-600">   IT UP</span>
            </motion.h1>
            <div className="text-[10px] font-mono tracking-[0.3em] text-red-500/50 uppercase mt-1 pl-1">
              Bearish Intelligence Terminal
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          {lastUpdated && (
             <div className="text-right flex items-center gap-2 text-red-500/40">
          ...
          <button 
             onClick={fetchData} 
             disabled={loading}
             className="h-8 w-8 flex items-center justify-center bg-white/5 border border-white/10 rounded hover:bg-white/10 hover:border-red-500/30 transition-all text-white/50"
          >
        ...
        <div className="col-span-8 flex flex-col min-h-0 bg-[#080808]/50 border border-white/5 rounded-2xl p-4 backdrop-blur-sm">
           <div className="flex-none mb-4 flex items-center justify-between">
              <h2 className="text-xs font-mono uppercase text-white/60 flex items-center gap-2">
                <IconNews size={16} className="text-red-500" /> Live Signals (Top 4)
              </h2>
           </div>
           ...
           <div className="flex-1 min-h-0 grid grid-cols-2 gap-4">
              {data ? data.live_signals.slice(0,4).map((signal, i) => (
                <motion.div
                  ...
                  className="bg-[#0c0c0c] border border-white/5 rounded-xl p-5 hover:border-red-500/30 transition-all group flex flex-col justify-between"
                >
                  <div>
                    <h3 className="text-lg font-bold text-white mb-2 leading-tight group-hover:text-red-400 transition-colors line-clamp-2">
                      {signal.title}
                    </h3>
        ...
        <div className="col-span-4 flex flex-col gap-6 min-h-0">
          ...
          <div className="flex-[0.4] min-h-0 bg-[#080808]/50 border border-white/5 rounded-2xl p-4 backdrop-blur-sm flex flex-col">
             <div className="flex-none mb-3">
                <h2 className="text-xs font-mono uppercase text-white/60 flex items-center gap-2">
                  <IconTarget size={16} className="text-red-500" /> High Conviction
                </h2>
             </div>
             
             <div className="flex-1 flex flex-col gap-2 overflow-hidden">
                {data ? data.top_picks.slice(0,3).map((pick, i) => (
                  <div key={i} className="flex-1 bg-[#0c0c0c] border border-white/5 rounded-lg px-4 flex items-center justify-between group hover:border-red-500/30 transition-colors">
                     <div>
                        <div className="flex items-center gap-2 mb-0.5">
                           ...
                           <span className={`text-[9px] font-black uppercase px-1 rounded ${
                             pick.action.toLowerCase() === 'sell' ? 'bg-red-500 text-black' : 'bg-green-500 text-black'
                           }`}>
                             {pick.action}
                           </span>
                        </div>
          ...
          <div className="flex-[0.6] min-h-0 bg-[#080808]/50 border border-white/5 rounded-2xl p-5 backdrop-blur-sm flex flex-col relative overflow-hidden">
             ...
             <IconActivity className="absolute bottom-[-20px] right-[-20px] text-white/[0.02]" size={150} />
             ...
             <div className="flex-1 flex flex-col gap-4 z-10">
                ...
                {/* AI Directive Box */}
                <div className="bg-red-950/20 border border-red-500/20 rounded-xl p-4 relative">
                   <div className="absolute top-3 left-3">
                      
                   </div>
                   <p className="text-sm font-medium text-red-100/90 italic leading-relaxed pl-6 line-clamp-4">
